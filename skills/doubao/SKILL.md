...
| List / remove MCP connectors | `doubao mcp list --json` / `doubao mcp remove <connector-id>` |
| Send with MCP tools | add `--mcp <connector-id>` (repeatable) to `sessions create`/`sessions send` |
...
- `--mcp <connector-id>` gives the model tools from a local MCP connector (register one with `mcp register` first; `mcp list` shows ids). Tool calls run locally without UI approval. Not combinable with `--attach`. Connectors are account-level; `mcp remove` disconnects and disables (there is no delete API).
